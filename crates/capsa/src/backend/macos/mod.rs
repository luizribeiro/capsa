...
use capsa_core::{
    BackendCapabilities, BackendVmHandle, ConsoleStream, Error, HostPlatform, HypervisorBackend,
...
// When SubprocessVmHandle is dropped, the tarpc client is dropped, which closes the
// transport pipes. The daemon detects EOF and exits, cleaning up all VMs automatically.
