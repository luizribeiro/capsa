...
/// Handles I/O port read operations from the guest.
fn handle_io_in(port: u16, data: &mut [u8], serial: &SerialDevice, rtc: &RtcDevice) {
...
/// Handles I/O port write operations from the guest.
fn handle_io_out(port: u16, data: &[u8], serial: &SerialDevice, rtc: &RtcDevice) {
    if serial.handles_io(port) {
...
pub fn run_vcpu(
    ...
    loop {
        if !running.load(Ordering::Relaxed) {
            // Use try_send to avoid blocking when multiple vCPUs exit simultaneously
        ...
        match vcpu.run() {
            ...
            Ok(VcpuExit::IoIn(port, data)) => handle_io_in(port, data, &serial, &rtc),
            Ok(VcpuExit::IoOut(port, data)) => handle_io_out(port, data, &serial, &rtc),
